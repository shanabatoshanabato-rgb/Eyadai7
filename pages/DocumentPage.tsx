...
export const DocumentPage: React.FC = () => {
  ...
  const handleConsult = async () => {
    ...
    try {
      ...
      1. SOURCE: Use authentic sources only: Holy Quran, Sahih Bukhari, Sahih Muslim.
      2. FORMAT: Respond ONLY in this JSON format:
         {
           ...
           "source": "The specific Verse/Hadith number and collection name used in ${targetLang}",
           "wisdom": "A short, beautiful spiritual reflection related to the topic in ${targetLang}"
         }
      3. TONE: Gentle, wise, and informative.
      4. LANGUAGE: Everything must be in ${targetLang}.
      
      Respond only with JSON.`;

      const response = await generateText(prompt, { 
        useSearch: useSearch,
        systemInstruction: "You are Eyad Al-Alem, a virtual Islamic guide who provides verified knowledge from the Quran and Sunnah. You prioritize authenticity and moderation."
      });
  ...
  const downloadAnswer = () => {
    if (!answer) return;
    const content = `الدليل الإسلامي - إياد\n\nالسؤال: ${question}\n\nالإجابة:\n${answer.body}\n\nالمصدر: ${answer.source}\n\nحكمة: ${answer.wisdom}`;
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-10 space-y-12 bg-emerald-50/20 dark:bg-transparent">
      {/* Hero Header */}
        ...
        <div className="relative z-10 space-y-8">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-50 dark:bg-emerald-900/30 rounded-full text-emerald-600 dark:text-emerald-400 text-xs font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-800/50">
              <Library className="w-4 h-4" /> Trusted Knowledge Hub
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in fade-in slide-in-from-bottom-12 duration-700">
          <div className="lg:col-span-8 bg-white dark:bg-slate-900 rounded-[3rem] p-10 md:p-14 border border-emerald-100 dark:border-slate-800 shadow-xl space-y-8">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-8">
              <h2 className="text-3xl font-black dark:text-white">{answer.title}</h2>
            ...
            <div className="prose prose-xl dark:prose-invert max-w-none">
              <div className="whitespace-pre-wrap dark:text-slate-300 text-slate-700 font-medium leading-relaxed">
                {answer.body}
              </div>
            </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group">
              <div className="relative z-10 space-y-6">
                ...
                <div className="pt-4 flex items-center gap-2 text-xs font-black uppercase tracking-widest opacity-60">
                  <Sparkles className="w-3 h-3" /> Spiritual Reflection
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-100 dark:border-slate-800">
               <h4 className="font-black text-xs uppercase tracking-widest text-slate-400 mb-6">Recent Consultations</h4>
                  ...
                  {[1, 2].map(i => (
                    <div key={i} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all cursor-pointer group">
                      ...
                      <div className="flex-grow">
                         <p className="text-sm font-bold dark:text-white truncate">The virtues of patience...</p>
                         <p className="text-[10px] text-slate-400 font-black uppercase">Quranic Guide</p>
                      </div>
